...
namespace SoftFluent.SocialEmailLogin
{
    public class YahooServiceProvider : AuthServiceProvider
    {
        ...
        protected override UserData GetUserData(HttpRequest httpRequest)
        {
            ...
            foreach (string key in httpRequest.Query.Keys.Where(key => key.StartsWith("openid.")))
            {
                data[key] = DecodeUrlParameter(httpRequest.Query[key]);
            }

            foreach (string key in httpRequest.Form.Keys.Where(key => key.StartsWith("openid.")))
            {
